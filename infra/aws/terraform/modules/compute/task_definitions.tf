...
# Snowflake Connector Task Definition
resource "aws_ecs_task_definition" "snowflake_connector" {
  count = contains(var.enabled_connectors, "snowflake") ? 1 : 0

  family                   = "omni-${var.customer_name}-snowflake-connector"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = var.task_cpu
  memory                   = var.task_memory
  ...
  container_definitions = jsonencode([{
    name      = "omni-snowflake-connector"
    image     = "ghcr.io/${var.github_org}/omni/omni-snowflake-connector:latest"
    essential = true
    ...
    logConfiguration = {
      ...
      options = {
        "awslogs-group"         = var.log_group_name
        "awslogs-region"        = var.region
        "awslogs-stream-prefix" = "snowflake-connector"
      }
    }

    environment = concat(local.connector_base_environment, [
      { name = "PORT", value = "4020" },
      { name = "CONNECTOR_HOST_NAME", value = "snowflake-connector" }
    ])
  ...
  tags = merge(local.common_tags, {
    Name = "omni-${var.customer_name}-snowflake-connector"
  })
}

# Paperless Connector Task Definition
resource "aws_ecs_task_definition" "paperless_connector" {
  count = contains(var.enabled_connectors, "paperless") ? 1 : 0

  family                   = "omni-${var.customer_name}-paperless-connector"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = var.task_cpu
  memory                   = var.task_memory
  ...
  container_definitions = jsonencode([{
    name      = "omni-paperless-connector"
    image     = "ghcr.io/${var.github_org}/omni/omni-paperless-connector:latest"
    essential = true
    ...
    logConfiguration = {
      ...
      options = {
        "awslogs-group"         = var.log_group_name
        "awslogs-region"        = var.region
        "awslogs-stream-prefix" = "paperless-connector"
      }
    }

    environment = concat(local.connector_base_environment, [
      { name = "PORT", value = "4015" },
      { name = "CONNECTOR_HOST_NAME", value = "paperless-connector" }
    ])
  ...
  tags = merge(local.common_tags, {
    Name = "omni-${var.customer_name}-paperless-connector"
  })
}
