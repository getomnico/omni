...
resource "aws_ecs_service" "google_ads_connector" {
  count = contains(var.enabled_connectors, "google_ads") ? 1 : 0

  name            = "omni-${var.customer_name}-google-ads-connector"
  cluster         = var.cluster_arn
  task_definition = aws_ecs_task_definition.google_ads_connector[0].arn
  launch_type     = "FARGATE"
  desired_count   = var.desired_count
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.google_ads_connector[0].arn
  }
...
resource "aws_ecs_service" "snowflake_connector" {
  count = contains(var.enabled_connectors, "snowflake") ? 1 : 0

  name            = "omni-${var.customer_name}-snowflake-connector"
  cluster         = var.cluster_arn
  task_definition = aws_ecs_task_definition.snowflake_connector[0].arn
  launch_type     = "FARGATE"
  desired_count   = var.desired_count
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.snowflake_connector[0].arn
  }
...
# Paperless Connector Service
resource "aws_ecs_service" "paperless_connector" {
  count = contains(var.enabled_connectors, "paperless") ? 1 : 0

  name            = "omni-${var.customer_name}-paperless-connector"
  cluster         = var.cluster_arn
  task_definition = aws_ecs_task_definition.paperless_connector[0].arn
  launch_type     = "FARGATE"
  desired_count   = var.desired_count
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.paperless_connector[0].arn
  }
