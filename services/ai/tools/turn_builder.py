...
every turn so that newly-loaded connector toolsets become available immediately
after the meta-tools (`tool_search`, `load_tool_set`) fire. This module owns the
...
def build_turn_tools(
    ...
    meta-tools. Connector tools are filtered to only those whose source_id has
    ...
    if connector_handler is not None:
        tools.extend(connector_handler.filtered_tools(loaded_toolsets))
    return tools
