-- Snowflake metadata connector and managed MCP provider.
...
CHECK (
  (integration_type = 'connector' AND source_type IN (
    ...
    'google_ads', 'darwinbox', 'windshift', 'salesforce', 'snowflake'
  ))
  OR (integration_type = 'remote_mcp' AND source_type ~ '^[a-z][a-z0-9_-]{1,49}$')
);
...
CHECK (provider IN (
  'google', 'slack', 'atlassian', 'github', 'notion', 'fireflies', 'hubspot',
  'microsoft', 'imap', 'clickup', 'linear', 'paperless_ngx', 'nextcloud',
  'google_ads', 'darwinbox', 'remote_mcp', 'windshift', 'salesforce', 'snowflake'
));
