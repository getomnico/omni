...
impl DriveClient {
    ...
    pub async fn list_changes(
        ...
        self.list_changes_internal(token, page_token, None).await
    }

    /// Like `list_changes` but scoped to a single shared drive.
    pub async fn list_changes_for_drive(
        &self,
        token: &str,
        page_token: &str,
        drive_id: &str,
    ) -> Result<DriveChangesResponse> {
        self.list_changes_internal(token, page_token, Some(drive_id))
            .await
    }

    /// Drive-scoped start page token. Required for per-drive incremental sync.
    pub async fn get_start_page_token_for_drive(
        &self,
        token: &str,
        drive_id: &str,
    ) -> Result<String> {
        ...
        let params = vec![("supportsAllDrives", "true"), ("driveId", &drive_id_owned)];
        ...
        let start_page_token = response_json["startPageToken"]
            .as_str()
            .ok_or_else(|| anyhow!("Missing startPageToken in drive-scoped response"))?;

        Ok(start_page_token.to_string())
    }

    /// Internal helper that optionally scopes to a drive.
    async fn list_changes_internal(
        &self,
        token: &str,
        page_token: &str,
        drive_id: Option<&str>,
    ...
    /// List all files (not just folders) in a single shared drive with the given cut-off.
    /// Uses `corpora=drive` and `driveId` to keep the query scoped.
    /// Paginates fully and returns all files.
    pub async fn list_files_in_drive(
        &self,
        auth: &GoogleAuth,
        user_email: &str,
        drive_id: &str,
