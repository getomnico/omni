...
mod tests {
    ...
    fn init_trace_subscriber() {
        ...
        INIT.call_once(|| {
            ...
            use opentelemetry_sdk::trace::{
                RandomIdGenerator, SdkTracerProvider, SimpleSpanProcessor,
            };
    ...
    async fn test_enqueue_traceparent_persisted() {
        ...
        // Start an active parent span so enqueue injects trace context.
        let parent_span = tracing::info_span!("test_enqueue_traceparent");
        async move {
            let event = make_event("run-tp", "doc-tp");
            let event_id = queue.enqueue(TEST_SOURCE_ID, &event).await.unwrap();

            // Query the stored row directly — traceparent must be non-null
            // and valid.
            let row: (Option<String>, Option<String>) = sqlx::query_as(
                "SELECT traceparent, tracestate FROM connector_events_queue WHERE id = $1",
            )
            .bind(&event_id)
            .fetch_one(&pool)
            .await
            ...
            // tracestate is optional (may be None when no tracestate set).
    ...
    async fn test_enqueue_batch_traceparent_persisted() {
        ...
        // Start an active parent span so enqueue_batch injects trace context.
        let parent_span = tracing::info_span!("test_enqueue_batch_traceparent");
        async move {
            let run_id = ulid::Ulid::new().to_string();
            let events: Vec<ConnectorEvent> = (0..2)
                .map(|i| make_event(&run_id, &format!("doc-batch-tp-{}", i)))
                .collect();
            let ids = queue.enqueue_batch(TEST_SOURCE_ID, &events).await.unwrap();
            assert_eq!(ids.len(), 2);

            // Query each row and verify valid traceparent.
            for id in &ids {
                let tp: Option<String> = sqlx::query_scalar(
                    "SELECT traceparent FROM connector_events_queue WHERE id = $1",
                )
                .bind(id)
                .fetch_one(&pool)
                .await
                ...
                assert!(tp.is_some(), "traceparent must be stored for batch item");
                assert!(
                    shared::telemetry::queue::is_valid_traceparent(&tp.unwrap()),
                    "stored traceparent must be valid W3C format"
                );
            }
        }
